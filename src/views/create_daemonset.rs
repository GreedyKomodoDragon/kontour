...
pub fn CreateDaemonSet() -> Element {
    ...
    let submit = move |_| {
        ...
        // Basic validation
        if name.is_empty() {
            error.set(Some("DaemonSet name is required".to_string()));
            return;
        }
        if image.is_empty() {
        ...
        spawn(async move {
            ...
            match daemonsets.create(&PostParams::default(), &daemonset).await {
                Ok(_) => {
                    navigate.push("/daemonsets");
                }
                Err(e) => {
                    error.set(Some(format!("Failed to create daemonset: {}", e)));
                }
            }
        });
    };
                ...
                div { class: "section-content",
                    div { class: "form-grid",
                        div { class: "form-group",
                            label { class: "form-label", "DaemonSet Name" }
                            input {
                                class: "form-input",
                                placeholder: "Enter daemonset name",
                                value: "{name}",
